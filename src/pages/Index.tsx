...
function Nav({ active, setActive }: { active: string; setActive: (s: string) => void }) {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50" style={{ background: "rgba(7,13,26,0.85)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(0,212,255,0.1)" }}>
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setActive("home")}>
          <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: "linear-gradient(135deg, #00d4ff, #00e5a0)" }}>
            <Icon name="Scale" size={16} className="text-[#070d1a]" />
          </div>
          <span className="font-oswald font-bold text-white text-lg tracking-wide">ОТМЕНА<span className="neon-text">ПРИКАЗА</span></span>
        </div>
        <nav className="hidden md:flex items-center gap-1">
          {links.map(l => (
            <button
              ...
              className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 font-golos ${active === l.id ? "text-[#00d4ff] bg-[rgba(0,212,255,0.1)]" : "text-slate-400 hover:text-white hover:bg-white/5"}`}
            >
              {l.label}
            </button>
          ))}
      ...
      <div className="md:hidden flex gap-1 px-4 pb-2 overflow-x-auto">
        {links.map(l => (
          <button
            key={l.id}
            onClick={() => setActive(l.id)}
            className={`px-3 py-1 rounded-full text-xs font-medium whitespace-nowrap transition-all duration-200 ${active === l.id ? "text-[#00d4ff] bg-[rgba(0,212,255,0.1)]" : "text-slate-400 bg-white/5"}`}
          >
            {l.label}
          </button>
        ))}
    ...
    <div className="min-h-screen pt-20">
      <section className="relative overflow-hidden pt-16 pb-24 px-4">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-20 left-1/4 w-64 h-64 rounded-full opacity-10 animate-float" style={{ background: "radial-gradient(circle, #00d4ff, transparent)" }} />
          <div className="absolute bottom-20 right-1/4 w-48 h-48 rounded-full opacity-8 animate-float" style={{ background: "radial-gradient(circle, #00e5a0, transparent)", animationDelay: "3s" }} />
        </div>
        <div className="max-w-4xl mx-auto text-center relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full mb-6" style={{ background: "rgba(0,212,255,0.1)", border: "1px solid rgba(0,212,255,0.25)" }}>
            <div className="w-2 h-2 rounded-full animate-pulse-neon" style={{ background: "#00d4ff" }} />
            <span className="text-xs font-golos text-[#00d4ff] tracking-widest uppercase font-semibold">Защита прав граждан</span>
          </div>
          <h1 className="font-oswald text-5xl md:text-7xl font-bold text-white mb-4 leading-tight">
            ОТМЕНИТЕ<br /><span className="gradient-text">СУДЕБНЫЙ ПРИКАЗ</span><br />БЕСПЛАТНО
          </h1>
          <p className="text-slate-400 text-lg md:text-xl max-w-2xl mx-auto mb-10 font-golos leading-relaxed">
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
          {stats.map((s, i) => (
            <div key={i} className="card-glass rounded-2xl p-6 text-center neon-border hover:border-[rgba(0,212,255,0.5)] transition-all duration-300 hover:-translate-y-1">
              <div className="w-12 h-12 rounded-xl mx-auto mb-4 flex items-center justify-center" style={{ background: "rgba(0,212,255,0.1)" }}>
                <Icon name={s.icon} size={22} className="text-[#00d4ff]" />
              </div>
              <div className="font-oswald text-3xl font-bold gradient-text mb-1">{s.value}</div>
              <div className="text-sm text-slate-400 font-golos">{s.label}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="py-16 px-4">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-12">
            ...
            <h2 className="font-oswald text-4xl font-bold text-white">ТРИ ПРОСТЫХ ШАГА</h2>
          </div>
          <div className="space-y-6">
            {steps.map((step, i) => (
              <div key={i} className="card-glass rounded-2xl p-6 neon-border hover:border-[rgba(0,212,255,0.4)] transition-all duration-300 flex gap-6 items-start">
                <div className="flex-shrink-0">
                  <div className="w-14 h-14 rounded-xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, rgba(0,212,255,0.2), rgba(0,229,160,0.1))", border: "1px solid rgba(0,212,255,0.3)" }}>
                    <Icon name={step.icon} size={24} className="text-[#00d4ff]" />
                  </div>
                </div>
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <span className="font-oswald text-4xl font-bold text-[rgba(0,212,255,0.2)]">{step.num}</span>
                    <h3 className="font-oswald text-xl font-semibold text-white">{step.title}</h3>
                  </div>
                  <p className="text-slate-400 font-golos leading-relaxed">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="py-16 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <div className="rounded-3xl p-10 relative overflow-hidden" style={{ background: "linear-gradient(135deg, rgba(0,212,255,0.1), rgba(0,229,160,0.05))", border: "1px solid rgba(0,212,255,0.2)" }}>
            <div className="absolute inset-0 pointer-events-none" style={{ background: "radial-gradient(ellipse at center, rgba(0,212,255,0.05), transparent 70%)" }} />
            <Icon name="Shield" size={40} className="text-[#00d4ff] mx-auto mb-4" />
            <h2 className="font-oswald text-3xl font-bold text-white mb-3">ГОТОВЫ ЗАЩИТИТЬ СВОИ ПРАВА?</h2>
            <p className="text-slate-400 font-golos mb-6">Создайте заявление прямо сейчас — это займёт не более 5 минут</p>
            <button
              onClick={() => setActive("constructor")}
...
function InfoPage() {
  ...
  const timeline = [
    { step: "Вынесение приказа", desc: "Суд выносит приказ без участия должника", color: "#00d4ff", icon: "FileText" },
    { step: "Направление копии", desc: "Должнику направляется копия по почте", color: "#00d4ff", icon: "Mail" },
    { step: "10-дневный срок", desc: "Срок для подачи возражения с момента получения", color: "#ff6b35", icon: "Clock" },
    { step: "Подача возражения", desc: "Вы подаёте возражение в суд", color: "#00e5a0", icon: "Send" },
    { step: "Отмена приказа", desc: "Суд обязан отменить приказ", color: "#00e5a0", icon: "CheckCircle" },
  ];
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 font-golos max-w-2xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-10">
          <div className="card-glass rounded-2xl p-6 neon-border">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "rgba(0,212,255,0.1)" }}>
                <Icon name="Scale" size={18} className="text-[#00d4ff]" />
              </div>
              <h3 className="font-oswald text-lg font-semibold text-white">Правовая основа</h3>
            </div>
            <p className="text-slate-400 font-golos text-sm leading-relaxed">
              Право на отмену судебного приказа закреплено в <span className="text-[#00d4ff]">статье 129 ГПК РФ</span>.
          ...
          <div className="card-glass rounded-2xl p-6 neon-border-green">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: "rgba(0,229,160,0.1)" }}>
                <Icon name="AlertTriangle" size={18} className="text-[#00e5a0]" />
              </div>
              <h3 className="font-oswald text-lg font-semibold text-white">Важно знать</h3>
            </div>
            <p className="text-slate-400 font-golos text-sm leading-relaxed">
              Если вы не получили копию приказа, <span className="text-[#00e5a0]">срок не начал течь</span>.
        ...
        <div className="card-glass rounded-2xl p-6 neon-border mb-8">
          <h2 className="font-oswald text-2xl font-bold text-white mb-6 flex items-center gap-3">
            <Icon name="GitBranch" size={22} className="text-[#00d4ff]" />
            СХЕМА ПРОЦЕДУРЫ
          </h2>
              ...
              <div key={i} className="flex gap-4 items-start">
                <div className="flex flex-col items-center">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0" style={{
                    background: `rgba(${item.color === "#00d4ff" ? "0,212,255" : item.color === "#00e5a0" ? "0,229,160" : "255,107,53"},0.15)`,
                    border: `1px solid ${item.color}`
                  }}>
                    <Icon name={item.icon} size={16} style={{ color: item.color }} />
                  </div>
                  {i < timeline.length - 1 && (
                    <div className="w-0.5 h-8 my-1 step-line opacity-30" />
                  )}
                </div>
                <div className="pb-6">
                  <div className="font-oswald font-semibold text-white text-sm">{item.step}</div>
                  <div className="text-slate-400 font-golos text-sm mt-0.5">{item.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div>
          <h2 className="font-oswald text-2xl font-bold text-white mb-6 flex items-center gap-3">
            <Icon name="HelpCircle" size={22} className="text-[#00d4ff]" />
            ЧАСТЫЕ ВОПРОСЫ
          </h2>
          <div className="space-y-3">
            {faqs.map((faq, i) => (
              <div
                key={i}
                className={`card-glass rounded-xl overflow-hidden transition-all duration-300 cursor-pointer ${openFaq === i ? "neon-border" : "border border-[#1a2d4a] hover:border-[rgba(0,212,255,0.2)]"}`}
                onClick={() => setOpenFaq(openFaq === i ? null : i)}
              >
                <div className="p-5 flex items-center justify-between gap-4">
                  <span className="font-golos font-semibold text-white text-sm">{faq.q}</span>
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ${openFaq === i ? "bg-[rgba(0,212,255,0.2)] text-[#00d4ff]" : "text-slate-500"}`}>
                    <Icon name={openFaq === i ? "ChevronUp" : "ChevronDown"} size={14} />
                  </div>
                </div>
                {openFaq === i && (
                  <div className="px-5 pb-5 animate-fade-in">
                    <div className="w-full h-px bg-[rgba(0,212,255,0.1)] mb-4" />
                    <p className="text-slate-400 font-golos text-sm leading-relaxed">{faq.a}</p>
                  </div>
                )}
              </div>
            ))}
  ...
  const renderField = (field: string) => {
    ...
    return (
      <div key={field} className="space-y-1.5">
        <label className="text-xs font-golos font-semibold text-slate-400 uppercase tracking-wider block">
          {fieldLabels[field]}
        </label>
        {isTextarea ? (
          <textarea
        ...
        {touched.has(field) && errors[field] && (
          <p className="text-xs font-golos flex items-center gap-1" style={{ color: "#ff6b35" }}>
            <Icon name="AlertCircle" size={12} />
            {errors[field]}
          </p>
        )}
        {touched.has(field) && !errors[field] && value && (
          <p className="text-xs font-golos flex items-center gap-1" style={{ color: "#00e5a0" }}>
      ...
      <div className="min-h-screen pt-24 pb-16 px-4">
        <div className="max-w-3xl mx-auto">
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-2xl mx-auto mb-4 flex items-center justify-center animate-scale-in" style={{ background: "linear-gradient(135deg, rgba(0,229,160,0.2), rgba(0,212,255,0.1))", border: "1px solid rgba(0,229,160,0.4)" }}>
              <Icon name="CheckCheck" size={28} className="text-[#00e5a0]" />
            </div>
            <h1 className="font-oswald text-3xl font-bold text-white mb-2">ЗАЯВЛЕНИЕ ГОТОВО!</h1>
            <p className="text-slate-400 font-golos">Скопируйте текст, распечатайте и подайте в суд</p>
          </div>

          <div className="card-glass rounded-2xl p-6 neon-border mb-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-oswald text-lg font-semibold text-white">Текст заявления</h2>
              <button
            ...
            <pre className="text-slate-300 font-golos text-sm leading-relaxed whitespace-pre-wrap p-4 rounded-xl" style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.05)" }}>
              {text}
            </pre>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div className="card-glass rounded-xl p-4 neon-border-green flex gap-3 items-start">
              <Icon name="Info" size={18} className="text-[#00e5a0] flex-shrink-0 mt-0.5" />
              <p className="text-sm text-slate-400 font-golos">Подайте заявление лично или направьте заказным письмом с уведомлением</p>
            </div>
            <div className="card-glass rounded-xl p-4 neon-border flex gap-3 items-start">
              <Icon name="Clock" size={18} className="text-[#00d4ff] flex-shrink-0 mt-0.5" />
              <p className="text-sm text-slate-400 font-golos">Суд обязан отменить приказ и направить вам копию определения об отмене</p>
            </div>
          </div>
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-2xl mx-auto">
        <div className="text-center mb-8">
          ...
          <h1 className="font-oswald text-3xl font-bold text-white">СОЗДАЙТЕ <span className="gradient-text">ЗАЯВЛЕНИЕ</span></h1>
        </div>

        <div className="mb-8">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs text-slate-500 font-golos">Шаг {step} из {totalSteps}</span>
            <span className="text-xs font-golos" style={{ color: "#00d4ff" }}>{Math.round(progress)}% заполнено</span>
          </div>
            ...
            {["Личные данные", "О приказе", "Основание"].map((label, i) => (
              <div key={i} className="flex flex-col items-center gap-1">
                <div
                  ...
                  style={{
                    background: step > i + 1 ? "#00e5a0" : step === i + 1 ? "#00d4ff" : "rgba(255,255,255,0.08)",
                    color: step > i + 1 || step === i + 1 ? "#070d1a" : "#64748b",
                    border: step === i + 1 ? "2px solid rgba(0,212,255,0.5)" : "none",
                    boxShadow: step === i + 1 ? "0 0 12px rgba(0,212,255,0.5)" : "none"
                  }}>
                  {step > i + 1 ? <Icon name="Check" size={12} /> : i + 1}
                </div>
                <span className={`text-xs font-golos hidden sm:block ${step === i + 1 ? "text-[#00d4ff]" : "text-slate-500"}`}>{label}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="card-glass rounded-2xl p-6 neon-border animate-fade-in">
          <h2 className="font-oswald text-xl font-bold text-white mb-6">
    ...
    <div className="min-h-screen pt-24 pb-16 px-4">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-slate-400 font-golos max-w-xl mx-auto">
        ...
        <div className="grid gap-6">
          {samples.map((s, i) => (
            <div key={i} className={`card-glass rounded-2xl overflow-hidden transition-all duration-300 ${open === i ? "neon-border" : "border border-[#1a2d4a] hover:border-[rgba(0,212,255,0.3)]"}`}>
              <div className="p-6 cursor-pointer" onClick={() => setOpen(open === i ? null : i)}>
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: "rgba(0,212,255,0.1)", border: "1px solid rgba(0,212,255,0.2)" }}>
                      <Icon name={s.icon} size={22} className="text-[#00d4ff]" />
                    </div>
                    <div>
                      ...
                      <h3 className="font-oswald text-xl font-semibold text-white">{s.title}</h3>
                      <p className="text-slate-400 font-golos text-sm mt-1">{s.desc}</p>
                    </div>
                  </div>
                  <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-all duration-300 ${open === i ? "bg-[rgba(0,212,255,0.2)]" : "bg-white/5"}`}>
                    <Icon name={open === i ? "ChevronUp" : "ChevronDown"} size={16} className={open === i ? "text-[#00d4ff]" : "text-slate-500"} />
                  </div>
                </div>
              </div>
              ...
              {open === i && (
                <div className="px-6 pb-6 animate-fade-in">
                  ...
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-xs text-slate-500 font-golos uppercase tracking-widest">Текст образца</span>
                    <button
                  ...
                  <pre className="text-slate-300 font-golos text-xs leading-relaxed whitespace-pre-wrap p-4 rounded-xl" style={{ background: "rgba(0,0,0,0.3)", border: "1px solid rgba(255,255,255,0.05)" }}>
                    {s.preview}
                  </pre>
                </div>
              )}
        ...
        <div className="mt-10 text-center">
          <div className="card-glass rounded-2xl p-6 neon-border inline-block max-w-md w-full">
            <Icon name="Wand2" size={28} className="text-[#00d4ff] mx-auto mb-3" />
            <p className="text-white font-golos font-semibold mb-2">Нужно персональное заявление?</p>
            <p className="text-slate-400 font-golos text-sm mb-4">Конструктор подставит ваши данные автоматически</p>
            <button
              onClick={() => setActive("constructor")}
...
function Footer({ setActive }: { setActive: (s: string) => void }) {
  return (
    <footer className="border-t mt-8" style={{ borderColor: "rgba(0,212,255,0.08)" }}>
      <div className="max-w-6xl mx-auto px-4 py-10">
        <div className="flex flex-col md:flex-row justify-between gap-8">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-7 h-7 rounded-lg flex items-center justify-center" style={{ background: "linear-gradient(135deg, #00d4ff, #00e5a0)" }}>
                <Icon name="Scale" size={14} className="text-[#070d1a]" />
              </div>
              <span className="font-oswald font-bold text-white">ОТМЕНА<span className="neon-text">ПРИКАЗА</span></span>
            </div>
            <p className="text-slate-500 text-sm font-golos max-w-xs">Сервис помощи гражданам в отмене судебных приказов</p>
          </div>
          <div className="flex gap-10">
            <div>
              <div className="text-xs text-slate-500 uppercase tracking-widest font-golos mb-3">Разделы</div>
              {["home", "info", "constructor", "samples"].map((id, i) => (
                <div key={id} className="mb-1.5">
                  <button onClick={() => setActive(id)} className="text-sm text-slate-400 hover:text-[#00d4ff] transition-colors font-golos">
            ...
            <div>
              <div className="text-xs text-slate-500 uppercase tracking-widest font-golos mb-3">Правовая база</div>
              <div className="text-sm text-slate-400 font-golos space-y-1.5">
        ...
        <div className="mt-8 pt-6 border-t flex flex-col md:flex-row items-center justify-between gap-2" style={{ borderColor: "rgba(255,255,255,0.05)" }}>
